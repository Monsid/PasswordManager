...
public class MyFunctions {
    ...
    public void saveFile() {
        ...
        try {
            ...
            if (!secretdir.exists()) {
                if (secretdir.mkdir()) {
                    secretdir = new File(secretdir + "/mysecrets.txt");
            ...
            writer.printf("%s" + "%n", PasswordManager.accounts.size());
            writer.printf("%s" + "%n", "");//had to calibrate with empty due to checking first taking 2 strings?

            for (Account acc : PasswordManager.accounts) {
    ...
    public void readFile() throws InvalidKeyException, IllegalBlockSizeException, BadPaddingException, NoSuchPaddingException, InvalidAlgorithmParameterException {
        ...
        File secretdir = new File("C:/Secrets/mysecrets.txt");
        if (secretdir.exists()) {

            try {
                File file = new File("C:/Secrets/mysecrets.txt");    //creates a new file instance
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    PasswordManager.accounts.add(nuacc);
    ...
    public void mergeSort(LinkedList<Account> accounts, int size) {

        //convert to basic array type
        Comparable[] mainarray = new Account[size];
    ...
    public void displayAccounts(TableView<Account> tableview) {
        ...
        ObservableList<Account> rows = FXCollections.observableArrayList();
        for (int i = 0; i < PasswordManager.accounts.size(); i++) {
            rows.add(PasswordManager.accounts.get(i));
        }
